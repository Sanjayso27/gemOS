...
long int3_handler(struct exec_context *ctx)
{
	...
	par->dbg->regs.entry_rip=ctx->regs.entry_rip-1;
	...
	}
	{
		...
		while(*((u64*)back)!=END_ADDR){
			if(*((u64*)back)!=(u64)par->dbg->end_handler){
	...
	if(chk==0 ){
		...
		*((u64 *) ctx->regs.entry_rsp)=ctx->regs.rbp;
		...
		par->dbg->regs.rdx=ctx->regs.rdx+8;
		ctx->regs.entry_rsp-=8;
		*((u64 *) ctx->regs.entry_rsp)=ctx->regs.rbp;
		par->dbg->bp_cnt--;
	}
	...
	if(ctx->dbg->head==NULL){
		...
		while(cur!=NULL){
			if(cur->addr==(u64)addr){
				chk=0;
				int in=0;
				for(int i=0;i<ctx->dbg->bp_cnt;i++){
					if(ctx->dbg->bp[i]==(u64)addr)in=1;
				}
				if((in==0)){
					...
					if(cur->end_breakpoint_enable!=flag)return -1;
					return 0;
				}
				break;
			}
...
int do_remove_breakpoint(struct exec_context *ctx, void *addr)
{
	...
	while(cur!=NULL){
		if(cur->addr==(u64)addr){
			// go through bp and check
			int ok=1;
			if(cur->end_breakpoint_enable==1){
				for(int i=0;i<ctx->dbg->bp_cnt;i++){
					if(ctx->dbg->bp[i]==(u64)addr)ok=0;
				}
				if(ok==0)return -1;
			}
...
s64 do_wait_and_continue(struct exec_context *ctx)
{
	...
	for(int i=0;i<=MAX_PROCESSES;i++){
		...
		if((child!=NULL) && ctx->pid==child->ppid){
			chk=1;
			break;
		}
	}
